...
> **持续时间**：2-12 周（因范围而异） | **智能体**：15-30+ | **门守**：Agents Orchestrator

---

## 目标

通过持续的 Dev↔QA 循环实现所有功能。每个任务在开始下一个任务之前都经过验证。这是大部分工作发生的地方 — 也是 NEXUS 编排提供最大价值的地方。

## 前置条件

- [ ] 阶段 2 质量门通过（基础已验证）
- [ ] Sprint Prioritizer 待办列表可用，带有 RICE 分数
- [ ] CI/CD 管道可运行
- [ ] 设计系统和组件库就绪
- [ ] API 脚手架与认证系统就绪
...
```
FOR EACH task IN sprint_backlog (按 RICE 分数排序):

  1. 将任务分配给适当的开发者智能体（参见分配矩阵）
  2. 开发者实现任务
  ...
  4. IF 裁决 == 通过:
       标记任务完成
       进入下一个任务
     ELIF 裁决 == 失败 AND 尝试次数 < 3:
       将 QA 反馈发送给开发者
       开发者修复特定问题
       返回步骤 3
     ELIF 尝试次数 >= 3:
       升级给 Agents Orchestrator
...
| **座舱控制** | XR Cockpit Interaction Specialist | XR Interface Architect | Evidence Collector |
...
| Whimsy Injector | 功能需要趣味/个性 | UX 审查发现机会 |
| Visual Storyteller | 需要视觉叙事内容 | 内容需要视觉资产 |
| Brand Guardian | 品牌一致性问题 | QA 发现品牌偏差 |
...
### 轨道 A：核心产品开发
```
管理者：Agents Orchestrator（Dev↔QA 循环）
智能体：Frontend Developer、Backend Architect、AI Engineer、
...
智能体：Growth Hacker、Content Creator、Social Media Strategist、
        App Store Optimizer
...
智能体：Evidence Collector、API Tester、Performance Benchmarker、
...
```
管理者：Brand Guardian
智能体：UI Designer、Brand Guardian、Visual Storyteller、
        Whimsy Injector
...
| 4 | 品牌一致性已验证（95%+ 遵守率） | Brand Guardian 审计 | ☐ |
| 5 | 无关键 Bug（零 P0/P1 未解决） | Test Results Analyzer 摘要 | ☐ |
| 6 | 所有验收标准已满足 | 逐任务验证 | ☐ |
| 7 | 所有 PR 的代码审查已完成 | Git 历史证据 | ☐ |

## 门决策

**门守**：Agents Orchestrator

- **通过**：功能完整的应用 → 激活阶段 4
- **继续**：需要更多冲刺 → 继续阶段 3
- **升级**：系统性问题 → Studio Producer 介入
